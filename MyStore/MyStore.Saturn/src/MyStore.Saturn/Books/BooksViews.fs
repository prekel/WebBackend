...
module Views =
                                ...
                                tr [] [
                                    ...
                                    td [] [
                                        a [ _class "button is-text"
                                            _href (Links.withId ctx o.id) ] [
                                            encodedText "Show"
                                        ]
                                        a [ _class "button is-text"
                                            _href (Links.edit ctx o.id) ] [
                                            encodedText "Edit"
                                        ]
                                        a [ _class "button is-text is-delete"
                                            attr "data-href" (Links.withId ctx o.id) ] [
                                            encodedText "Delete"
                                        ]
                                    ]
                                ]
    ...
    let show (ctx: HttpContext) (o: Book) =
        let cnt =
            [ div [ _class "container " ] [
                h2 [ _class "title" ] [
                    encodedText "Show Book"
                ]

                ul [] [
                    li [] [
                        strong [] [ encodedText "Id: " ]
                        encodedText (string o.id)
                    ]
                    li [] [
                        strong [] [ encodedText "Title: " ]
                        encodedText (string o.title)
                    ]
                    li [] [
                        strong [] [ encodedText "Author: " ]
                        encodedText (string o.author)
                    ]
                ]
                a [ _class "button is-text"
                    _href (Links.edit ctx o.id) ] [
    ...
    let private form (ctx: HttpContext) (o: Book option) (validationResult: Map<string, string>) isUpdate =
        ...
        let cnt =
            [ div [ _class "container " ] [
                form [ _action (if isUpdate then Links.withId ctx o.Value.id else Links.index ctx)
                       _method "post" ] [
